...
import type { ChangeClassNames, VersionChanges } from "./changes.js";
import { annotateSidebars } from "./changes.js";
...
export interface Materialization {
  ...
  /**
   * What each version added and changed, oldest first. Pass it through to the site so a
   * `sidebarItemsGenerator` can mark autogenerated entries too — the ones this adapter cannot reach.
   */
  readonly changes: readonly VersionChanges[];
...
export interface MaterializeOptions {
  ...
  /**
   * Classes to hang on sidebar entries naming a page the version added or changed. Off unless set, because
   * it rewrites generated sidebars, and a site that does not style the classes gains nothing from them.
   */
  readonly changeClassNames?: ChangeClassNames | undefined;
...
export function materialize(overlay: Overlay<DocusaurusMeta>, options: MaterializeOptions = {}): Materialization {
  ...
  // Slugs the version examined one step earlier serves. `undefined` on the first iteration, which is what
  // keeps the oldest version unmarked: it owns every one of its pages, so comparing it against nothing
  // would call the whole tree new. `overlay.versions` runs oldest first with the channel last, and a
  // second channel is already an error above, so the previous element is always the predecessor.
  let servedBefore: ReadonlySet<string> | undefined;

  for (const version of versions) {
    const pages = overlay.getPages(version.id);
    const served = new Set(pages.map(page => slugKey(page.slug)));
    ...
    for (const page of pages) {
      ...
      // Owning the file *is* the diff. A renamed page counts as added, because its slug is one the
      // predecessor did not serve — which is exactly what a reader arriving at that URL experiences.
      if (!page.inherited && servedBefore !== undefined) {
        (servedBefore.has(slugKey(page.slug)) ? changed : added).add(docId);
      }
    }
    ...
    if (sidebars === undefined) {
      ...
      const marked = options.changeClassNames === undefined ? merged : annotateSidebars(merged, { added, changed }, options.changeClassNames);
      files.push({ kind: "write", path: version.sidebarsPath, contents: `${JSON.stringify(marked, undefined, 2)}\n` });
    }

    changes.push({ version: version.id, added: [...added].sort(), changed: [...changed].sort() });
    servedBefore = served;
  }
