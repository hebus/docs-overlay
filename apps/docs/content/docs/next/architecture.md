...
The dotted line is the one edge that is not a hard dependency: the command line declares
`docs-overlay-docusaurus` as an **optional** peer and imports it lazily, so `cut` and `prune` work on a
repository that has never heard of Docusaurus. Nothing points upwards anywhere on this drawing, and that
is the whole rule.
